...
{
    defineTypeNameAndDebug(McCowanNew, 0);
...
Foam::scalar Foam::waveModels::McCowanNew::eta
(
    const scalar H,
    const scalar h,
    const scalar x,
...
Foam::vector Foam::waveModels::McCowanNew::mn
(
    const scalar H,
    const scalar h
) const
{
    ...
    // m
    // scalar xin = 1;
    scalar xin = sqrt(3.0) * alpha;  // for faster convergence
    scalar m = newtonRapsonF1(xin, H, h);

    // n
    // scalar c1 = sin(m + (1.0 + (2.0*H/(3.0*h))));
    scalar a = m * (1.0 + 2.0/3.0 * alpha);
    scalar c1 = sin(a);
...
Foam::scalar Foam::waveModels::McCowanNew::newtonRapsonF1
(
    const scalar x0,
    const scalar H,
    const scalar h
) const
{
    ...
    while (iter <= N)
    {
        ...
        */
        ...
        scalar alpha = H / h;
        scalar a = x * (1.0 + 2.0/3.0 * alpha);
        scalar b = 0.5*x*(1.0 + alpha);
        scalar c1 = sin(a);
        scalar c2 = 1.0/tan(b);
        scalar c3 = 1.0/sin(b);

        scalar fx = (2.0/3.0)*sqr(c1) - x * alpha * c2;
        ...
        */
        ...
        scalar fprime = (4.0/3.0) * c1 * (1.0 + 2.0/3.0 * alpha) * cos(a) - alpha * c2 + b * alpha * sqr(c3);
...
Foam::scalar Foam::waveModels::McCowanNew::newtonRapsonF2
(
    const scalar x0,
    const scalar H,
    const scalar h,
...
) const
{
    ...
    while (iter <= N)
    {
        ...
        */
        ...
        scalar c2 = sin(a);
        scalar tmpA = c2;
        scalar tmpB = c1 + cosh(m * xa / h);
        scalar fx = x - h * n / m * tmpA / tmpB;
        ...
        scalar fprime = 1 - sqr(c2) * n / sqr(c3) - n * c1 / c3;
...
Foam::vector Foam::waveModels::McCowanNew::Uf
(
    const scalar H,
    const scalar h,
    const scalar x,
...
void Foam::waveModels::McCowanNew::setLevel
(
    const scalar t,
    const scalar tCoeff,
    scalarField& level
...
Foam::waveModels::McCowanNew::McCowanNew
(
...
bool Foam::waveModels::McCowanNew::readDict(const dictionary& overrideDict)
{
...
void Foam::waveModels::McCowanNew::setVelocity
(
    const scalar t,
    const scalar tCoeff,
...
void Foam::waveModels::McCowanNew::info(Ostream& os) const
{
    solitaryWaveModel::info(os);
    vector vec = this->mn(waveHeight_, waterDepthRef_);
    scalar mm = vec[0];
    scalar nn = vec[1];

    os << "waveHeight = " << waveHeight_ << nl;
    os << "waterDepthRef = " << waterDepthRef_ << nl;
