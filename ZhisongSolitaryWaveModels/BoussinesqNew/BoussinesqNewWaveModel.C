...
#include "BoussinesqNewWaveModel.H"
...
{
    defineTypeNameAndDebug(BoussinesqNew, 0);
...
Foam::scalar Foam::waveModels::BoussinesqNew::eta
(
    const scalar H,
    const scalar h,
    const scalar x,
...
Foam::vector Foam::waveModels::BoussinesqNew::Deta
(
    const scalar H,
    const scalar h,
    const scalar x,
...
) const
{
    ...
    scalar Xa = -C*t + ts - X0 + x*cos(theta) + y*sin(theta);
    scalar expTerm = exp(2.0*a*Xa);
    scalar expTerm4 = exp(4.0*a*Xa);
    scalar expTerm6 = exp(6.0*a*Xa);
...
*/
    ...
    deta[1] =
        2*a*b*(expTerm4 - 4*expTerm + 1)
       /pow4(1 + expTerm);

    deta[2] =
       -4*sqr(a)*b*(expTerm6 - 11*expTerm4 + 11*expTerm - 1)
       /pow5(1 + expTerm);
    return deta;
}


Foam::vector Foam::waveModels::BoussinesqNew::Uf
(
    const scalar H,
    const scalar h,
    const scalar x,
...
void Foam::waveModels::BoussinesqNew::setLevel
(
    const scalar t,
    const scalar tCoeff,
    scalarField& level
...
void Foam::waveModels::BoussinesqNew::setVelocity
(
    const scalar t,
    const scalar tCoeff,
...
Foam::waveModels::BoussinesqNew::BoussinesqNew
(
    ...
    solitaryWaveModel(dict, mesh, patch, false)
{
    ...
    {
        readDict(dict);
        // localWaterDepth = dict.getScalar("waterDepth");
        // dict.getScalar("waterDepth");
...
bool Foam::waveModels::BoussinesqNew::readDict(const dictionary& overrideDict)
{
...
void Foam::waveModels::BoussinesqNew::info(Ostream& os) const
{
